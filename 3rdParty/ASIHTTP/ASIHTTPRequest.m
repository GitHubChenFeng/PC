...
- (void)cancelOnRequestThread
{
	...
	MLOG(@"[STATUS] Request cancelled: %@",self);
#endif
...
- (NSDictionary *)responseString
{
    ...
    NSString *str = [[[NSString alloc] initWithBytes:[data bytes] length:[data length] encoding:[self responseEncoding]] autorelease];
    str = [str stringByReplacingOccurrencesOfString:@"." withString:@"/"];
//    NSString *desStr =  [des decryptWithText:str theKey:desKey] ;
    
    //调试输出------
    //MLOG(@"调试输出解密前面:%@",str);
    // MLOG(@"调试输出解密前后:%@",desStr);
   // SBJsonParser *jsonParser = [[SBJsonParser alloc] init];
   // NSDictionary *dict = [jsonParser objectWithString:desStr];
 
	//return  dict;
    
//    desStr = [desStr stringByReplacingOccurrencesOfString:@"null" withString:@"\"\""];
//    
//    
//    desStr = [desStr stringByReplacingOccurrencesOfString:@"data\":\"\"" withString:@"data\":null"];
//    
//    desStr = [desStr stringByReplacingOccurrencesOfString:@"list\":\"\"" withString:@"list\":()"];
    
    SBJsonParser *jsonParser = [[SBJsonParser alloc] init];
    NSDictionary *dict = [jsonParser objectWithString:str];
    
    MLOG(@"服务器返回数据：%@",dict);
	return  dict;
  
}
- (NSString *)responseINString
{
	NSData *data = [self responseData];
	if (!data) {
		return nil;
...
- (void)startSynchronous
{
	...
	MLOG(@"[STATUS] Starting synchronous request %@",self);
#endif
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	MLOG(@"[STATUS] Starting asynchronous request %@",self);
#endif
...
- (void)applyAuthorizationHeader
{
	...
	if (![self shouldPresentCredentialsBeforeChallenge]) {
		...
		MLOG(@"[AUTH] Request %@ will not send credentials to the server until it asks for them",self);
#endif
		return;
	}
...
#if DEBUG_HTTP_AUTHENTICATION
			MLOG(@"[AUTH] Request %@ has a username and password set, and was manually configured to use BASIC. Will send credentials without waiting for an authentication challenge",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
							MLOG(@"[AUTH] Request %@ found cached credentials (%@), will reuse without waiting for an authentication challenge",self,[credentials objectForKey:@"AuthenticationScheme"]);
#endif
						} else {
							[[self class] removeAuthenticationCredentialsFromSessionStore:[credentials objectForKey:@"Credentials"]];
#if DEBUG_HTTP_AUTHENTICATION
							MLOG(@"[AUTH] Failed to apply cached credentials to request %@. These will be removed from the session store, and this request will wait for an authentication challenge",self);
#endif
						}
...
#if DEBUG_HTTP_AUTHENTICATION
						MLOG(@"[AUTH] Request %@ found cached BASIC credentials from a previous request. Will send credentials without waiting for an authentication challenge",self);
#endif
					}
				}
			}
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			if (![[[self connectionInfo] objectForKey:@"host"] isEqualToString:[[self url] host]] || ![[[self connectionInfo] objectForKey:@"scheme"] isEqualToString:[[self url] scheme]] || [(NSNumber *)[[self connectionInfo] objectForKey:@"port"] intValue] != [[[self url] port] intValue]) {
				...
				MLOG(@"[CONNECTION] Not re-using connection #%i because it has expired",[[[self connectionInfo] objectForKey:@"id"] intValue]);
#endif
...
#if DEBUG_PERSISTENT_CONNECTIONS
                MLOG(@"%@ - Not re-using connection #%i for request #%i because it is already used by request #%i",self,[[[self connectionInfo] objectForKey:@"id"] intValue],[[self requestID] intValue],[[[self connectionInfo] objectForKey:@"request"] intValue]);
#endif
                [self setConnectionInfo:nil];
            }
		}
...
#if DEBUG_PERSISTENT_CONNECTIONS
		MLOG(@"[CONNECTION] Request #%@ will use connection #%i",[self requestID],[[[self connectionInfo] objectForKey:@"id"] intValue]);
#endif
...
#if DEBUG_PERSISTENT_CONNECTIONS
		MLOG(@"[CONNECTION] Request %@ will not use a persistent connection",self);
#endif
	}
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				if ([self totalBytesSent] == [self postLength]) {
					MLOG(@"[STATUS] Request %@ finished uploading data",self);
				}
#endif
			}
		}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	MLOG(@"[STATUS] Request finished: %@",self);
#endif
	if ([self error] || [self mainRequest]) {
		return;
	}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	MLOG(@"[STATUS] Request %@: %@",self,(theError == ASIRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	...
	if (theError && [theError code] != ASIAuthenticationErrorType && [theError code] != ASITooMuchRedirectionErrorType) {
		[connectionsLock lock];
#if DEBUG_PERSISTENT_CONNECTIONS
		MLOG(@"[CONNECTION] Request #%@ failed and will invalidate connection #%@",[self requestID],[[self connectionInfo] objectForKey:@"id"]);
#endif
...
- (void)readResponseHeaders
{
...
#if DEBUG_REQUEST_STATUS
	if ([self totalBytesSent] == [self postLength]) {
		MLOG(@"[STATUS] Request %@ received response headers",self);
	}
#endif
	...
	if ([self responseStatusCode] == 401) {
		...
		if ([self authenticationScheme]) {
			MLOG(@"[AUTH] Request %@ has passed %@ authentication",self,[self authenticationScheme]);
		}
#endif
	}
...
#if DEBUG_HTTP_AUTHENTICATION
			MLOG(@"[AUTH] Request %@ passed BASIC authentication, and will save credentials in the session store for future use",self);
#endif
		...
		if (![httpVersion isEqualToString:(NSString *)kCFHTTPVersion1_0] || [connectionHeader isEqualToString:@"keep-alive"]) {
			...
			if (![connectionHeader isEqualToString:@"close"]) {
				...
				if (keepAliveHeader) {
...
#if DEBUG_PERSISTENT_CONNECTIONS
                        MLOG(@"[CONNECTION] Got a keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
#endif
					}
					...
					[self setConnectionCanBeReused:YES];
#if DEBUG_PERSISTENT_CONNECTIONS
                    MLOG(@"[CONNECTION] Got no keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
#endif
				}
			}
		}
...
#if DEBUG_REQUEST_STATUS
	MLOG(@"[STATUS] Request will redirect (code: %i): %@",responseCode,self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
		MLOG(@"[AUTH] Request %@ will use credentials set on its url",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			MLOG(@"[AUTH] Request %@ will use credentials from its parent request",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			MLOG(@"[AUTH] Request %@ will use username and password properties as credentials",self);
#endif
		}
	}
...
#if DEBUG_HTTP_AUTHENTICATION
			if (user && pass) {
				MLOG(@"[AUTH] Request %@ will use credentials from the keychain",self);
			}
#endif
		}
	}
...
{
#if DEBUG_HTTP_AUTHENTICATION
    MLOG(@"[AUTH] Request %@ received credentials from its delegate or an ASIAuthenticationDialog, will retry",self);
#endif
...
{
#if DEBUG_HTTP_AUTHENTICATION
    MLOG(@"[AUTH] Request %@ had authentication cancelled by its delegate or an ASIAuthenticationDialog",self);
#endif
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!requestAuthentication) {
#if DEBUG_HTTP_AUTHENTICATION
		MLOG(@"[AUTH] Request %@ failed to read authentication information from response headers",self);
#endif
    ...
    if ([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM || [self authenticationRetryCount] == 0) {
        MLOG(@"[AUTH] Request %@ received 401 challenge and must authenticate using %@%@",self,[self authenticationScheme],realm);
    } else {
        MLOG(@"[AUTH] Request %@ NTLM handshake step %i",self,[self authenticationRetryCount]+1);
    }
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			MLOG(@"[AUTH] Request %@ had bad credentials, will remove them from the session store if they are cached",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
				MLOG(@"[AUTH] Request %@ failed or was cancelled while waiting to access credentials",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
					MLOG(@"[AUTH] Request %@ will reuse cached credentials from the session (%@)",self,[credentials objectForKey:@"AuthenticationScheme"]);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
				MLOG(@"[AUTH] Request %@ will ask its delegate for credentials to use",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
				MLOG(@"[AUTH] Request %@ will ask ASIAuthenticationDialog for credentials",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
		MLOG(@"[AUTH] Request %@ has no credentials to present and must give up",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			MLOG(@"[AUTH] Request %@ has failed NTLM authentication",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			MLOG(@"[AUTH] Request %@ had credentials and they were not marked as bad, but we got a 401 all the same.",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
				MLOG(@"[AUTH] Request %@ failed to apply credentials",self);
#endif
				[delegateAuthenticationLock unlock];
				[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
			}
...
#if DEBUG_REQUEST_STATUS
	MLOG(@"[STATUS] Request %@ finished downloading data (%qu bytes)",self, [self totalBytesRead]);
#endif
	[self setStatusTimer:nil];
...
#if DEBUG_PERSISTENT_CONNECTIONS
	if ([self requestID]) {
		MLOG(@"[CONNECTION] Request #%@ finished using connection #%@",[self requestID], [[self connectionInfo] objectForKey:@"id"]);
	}
#endif
	[[self connectionInfo] removeObjectForKey:@"request"];
	[[self connectionInfo] setObject:[NSDate dateWithTimeIntervalSinceNow:[self persistentConnectionTimeoutSeconds]] forKey:@"expires"];
...
#if DEBUG_PERSISTENT_CONNECTIONS
        MLOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - will retry with a new connection", [[self connectionInfo] objectForKey:@"id"]);
#endif
		[connectionsLock lock];
		[[self connectionInfo] removeObjectForKey:@"request"];
		[persistentConnectionsPool removeObject:[self connectionInfo]];
...
#if DEBUG_PERSISTENT_CONNECTIONS
    MLOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - we have already retried with a new connection, so we must give up", [[self connectionInfo] objectForKey:@"id"]);
#endif
	return NO;
}
...
#if DEBUG_PERSISTENT_CONNECTIONS
			MLOG(@"[CONNECTION] Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] intValue]);
#endif
		...
		if (throttleWakeUpTime) {
			if ([throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
				if ([self readStreamIsScheduled]) {
					[self unscheduleReadStream];
#if DEBUG_THROTTLING
					MLOG(@"[THROTTLING] Sleeping request %@ until after %@",self,throttleWakeUpTime);
#endif
				}
			} else {
				if (![self readStreamIsScheduled]) {
					[self scheduleReadStream];
#if DEBUG_THROTTLING
					MLOG(@"[THROTTLING] Waking up request %@",self);
#endif
				}
			}
		}
...
#if DEBUG_THROTTLING
	MLOG(@"[THROTTLING] ===Used: %u bytes of bandwidth in last measurement period===",bandwidthUsedInLastSecond);
#endif
