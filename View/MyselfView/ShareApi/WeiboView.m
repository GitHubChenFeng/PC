...
#define WeiboBGViewNumlabelHeight   30.f    // 渐变图标的高度，为了显示剩余多少字
...
- (void)createUpBar
{
    ...
    sendBtn.frame = CGRectMake(self.frame.size.width - 15.f - pngImage.size.width, WeiboBGViewUpBarHeight/2 - pngImage.size.height/2, pngImage.size.width, pngImage.size.height);
    [sendBtn setBackgroundImage:pngImage forState:UIControlStateNormal];
    [sendBtn setTag:1];
    [sendBtn addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    [_bgView addSubview:sendBtn];
    ...
    titleLabel.frame = CGRectMake(0.f, 0.f, ScreenWidth, WeiboBGViewUpBarHeight);
    titleLabel.text = @"评论";
...
{
    _textView = [[UITextView alloc]initWithFrame:CGRectMake(0.f, WeiboBGViewUpBarHeight + 1.f, ScreenWidth, WeiboBGViewHeight - WeiboBGViewNumlabelHeight - WeiboBGViewUpBarHeight)];
...
- (void)createBgView
{
    _bgView = [[UIView alloc]initWithFrame:CGRectMake(0.f, self.frame.size.height - WeiboBGViewHeight, ScreenWidth, WeiboBGViewHeight)];
...
//    }
    ...
    changeTypeBtn.frame = CGRectMake(10, 0, imgFace.size.width, imgFace.size.height);
...
- (void)changeType
{
    ...
    NSLog(@"hui === %f",_bgView.frame.origin.y);
    NSLog(@"value === %f",value);
...
- (void)hidefaceView
{
    [UIView animateWithDuration:0.25 animations:^{
        ...
        faceView.frame = CGRectMake(0.f,self.frame.size.height, ScreenWidth, WeiboBGViewHeight);
        _bgView.frame = CGRectMake(0.f,self.frame.size.height, ScreenWidth, WeiboBGViewHeight);
    } completion:^(BOOL finished) {
        [[NSNotificationCenter defaultCenter] removeObserver:self name:UIKeyboardWillShowNotification object:nil];
        [[NSNotificationCenter defaultCenter] removeObserver:self name:UIKeyboardWillHideNotification object:nil];
        [faceView removeFromSuperview];
...
- (void)buttonClick:(UIButton *)button
{
    ...
    if (button.tag == 1) {
        if ([delegate respondsToSelector:@selector(weiboViewSendComment:)]) {
            [delegate weiboViewSendComment:_textView.text];
        }
    }
}
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:animationDuration animations:^{
        _bgView.frame = CGRectMake(0.f, self.frame.size.height - WeiboBGViewHeight - keyboardRect.size.height, ScreenWidth, WeiboBGViewHeight);
    }];
...
- (void)keyboardWillHide:(NSNotification *)notification {
    ...
    if (flag) {
        ...
        [UIView animateWithDuration:animationDuration animations:^{
            _bgView.frame = CGRectMake(0.f,self.frame.size.height - WeiboBGViewHeight - 216.f, ScreenWidth, WeiboBGViewHeight); //
        }];
    }
}
