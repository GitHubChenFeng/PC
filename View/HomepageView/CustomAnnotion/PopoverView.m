...
//bottom color white in gradient bg [UIColor colorWithWhite:0.980f alpha:kBoxAlpha]
#define kGradientBottomColor RGB(34.0f, 172.0f, 56.0f)//绿色底色

//top color white value in gradient bg [UIColor colorWithWhite:1.f alpha:kBoxAlpha]
#define kGradientTopColor RGB(34.0f, 172.0f, 56.0f)

//蓝色底色
#define kBuleGradientBottomColor RGB(0.0f, 160.0f, 233.0f)
#define kBuleGradientTopColor  RGB(0.0f, 160.0f, 233.0f)
...
- (void)dealloc {
    ...
    [upTagButton release],upTagButton=nil;
    [downTagButton release],downTagButton=nil;
    self.contentView = nil;
    self.titleView = nil;
    // dufu add 2013.12.12  用于内存释放
    if (dividerRects) {
        [dividerRects release], dividerRects = nil;
    }
    [super dealloc];
}
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(UIView *)cView {
    ...
    //新增一个上面遮罩按钮，点击隐藏弹出菜单
    if (upTagButton == nil) {
        upTagButton = [[UIButton alloc]init];
        upTagButton.backgroundColor = [UIColor clearColor];
        int contentY = contentView.frame.origin.y - 6;
//        CGFloat contentH = contentFrame.size.height;
        upTagButton.frame = CGRectMake(0, 0, window.frame.size.width, contentY);
        
    }
    [upTagButton addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
    [window addSubview:upTagButton];
    
    //新增一个下面遮罩按钮，点击隐藏弹出菜单
    if (downTagButton == nil) {
        downTagButton = [[UIButton alloc]init];
        downTagButton.backgroundColor = [UIColor clearColor];
        int contentY = contentView.frame.origin.y;
        int contentH = contentFrame.size.height;
        downTagButton.frame = CGRectMake(0, contentY+contentH, window.frame.size.width, window.frame.size.height - contentY - contentH);
        
    }
    [downTagButton addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
    [window addSubview:downTagButton];
...
- (void)dismiss {
    [UIView animateWithDuration:0.3f animations:^{
        ...
        [upTagButton removeFromSuperview];
        [downTagButton removeFromSuperview];
...
- (void)drawRect:(CGRect)rect
{
    ...
    //// 设置底色颜色
    if (self.isSetBuleColor) {
        gradientColors = [NSArray arrayWithObjects:
                                   (id)kBuleGradientTopColor.CGColor,
                                   (id)kBuleGradientBottomColor.CGColor, nil];
    }else{
        gradientColors = [NSArray arrayWithObjects:
